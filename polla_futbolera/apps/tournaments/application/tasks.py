...
def run_sync_match_results(window_minutes: int = 300) -> dict:
    ...
    for match in candidatos.iterator():
        ...
        actualizados += 1
        logger.info("run_sync_match_results: match %s actualizado %s-%s", match.id, result_dto.home_score, result_dto.away_score)

    logger.info("run_sync_match_results: procesados=%d actualizados=%d", candidatos.count(), actualizados)
    return {"procesados": candidatos.count(), "actualizados": actualizados}
