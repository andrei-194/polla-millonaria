...
class PredictionService:
    def create_or_update_prediction(self, dto: CreatePredictionDTO) -> Prediction:
        ...
        if now >= match.match_date - timedelta(minutes=deadline_minutes):
    ...
    def get_group_predictions(
        self, match_id: int, group_id: int, requesting_user_id: int
    ):
        match = Match.objects.get(id=match_id)
        now = timezone.now()
        deadline_minutes = settings.PREDICTION_DEADLINE_MINUTES

        # Before deadline: only show own prediction
        if now < match.match_date - timedelta(minutes=deadline_minutes):
            return Prediction.objects.filter(
                match_id=match_id, group_id=group_id, user_id=requesting_user_id
            ).select_related("user")
